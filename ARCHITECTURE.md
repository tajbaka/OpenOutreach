...
**Gmail worker diagnostics (2026-09-18):** `gmail/worker_logging.py` provides a dedicated account-filtered rotating file sink during `run_gmail_worker`. Lifecycle and task events include PID, account, stage and task identity; fatal queue/startup/recovery errors retain exception chains and stack locations while excluding raw messages, source lines, locals and message/token payloads. Known failure reasons use fixed labels. The command re-raises crashes, closes its log handler, and retains instance-guard cleanup. Supervisor restart alerts include the local log path. Rotation is 5 MB with three backups per mailbox. Logging starts when the management command runs, so Django/import failures before that point and forced process termination may leave no fatal record. Sending, retry and account-routing policies are unchanged.

**Completed login race (2026-09-18):** The persistent login path checks the exact HTTPS LinkedIn feed before clicking submit and after a Playwright click timeout. A completed manual login can therefore survive a stale Sign in locator. It retains the ten-minute feed verification wait, using DOM readiness, and propagates unresolved login timeouts and unexpected browser errors. No session storage, account scope, or outbound behavior changes.

**Email-only supervisor (2026-09-17):** `ENABLE_LINKEDIN_DAEMON` defaults to true. When false, `_start_runtime_worker` launches `manage.py run_email_enrichment --operator <local canonical operator>` instead of the argument-free LinkedIn daemon, while the existing mapped Gmail worker continues independently. Feed scheduling is suppressed even when its own flag remains true. All initial, crash-recovery, Git-update and DB-requested child launches use this selection and the existing emergency-stop launch gate. The browserless enrichment command runs the existing worker synchronously under a sender-specific process guard with `include_phone=False`; claims and stale recovery retain exact operator scope, due dates and saved provider request IDs. Unexpected loop failures exit for supervisor recovery. Direct argument-free `manage.py` exits before startup checks/DB initialization/browser login when disabled. Restart the entire supervisor to reload `.env`; no automatic live-process migration is implied. Gmail follow-ups already scheduled and new steps produced by those sends continue under existing auth, stop, schedule and delivery checks. LinkedIn acceptance detection, inbound-message collection, new LinkedIn-triggered handoffs, node-heartbeat reporting and shared phone enrichment pause with the LinkedIn daemon. No campaign/Task mutation, migration, provider retry change or new send behavior is introduced by the mode switch.

**Personalized-invite quota handling (2026-09-17):** `actions/connect.py` recognizes LinkedIn's visible `out of free custom notes` message in invite dialogs and the `interop-outlet` shadow host before/after Add-a-note. It reports the actual restriction and raises the existing `ReachedConnectionLimit`; the task handler marks the sender's connect lane exhausted for today and schedules the next active start without incrementing missing-control attempts or disqualifying the lead. Existing versioned-delivery failure handling preserves frozen copy. No note-free fallback is allowed, and the existing in-process cooldown is not proof of LinkedIn's quota reset or a persistent account stop.

**Optional private reply guidance (2026-09-17):** The manual LinkedIn reply modal adds an optional, stable-ID, 500-character AI instructions input. The signed private draft parser snapshots this field, includes it in new request hashes, and carries it in Check/Retry action values so recovery reuses the same guidance and saved result. Editing or clearing guidance on Retry creates a new instruction-specific request instead. It is transient modal/job input, not Lead metadata, campaign copy, a new DB column, or part of manual-send submission. The private prompt separates operator guidance from untrusted conversation/profile/template data and reads the pure canonical calendar module's five Arian URLs plus event names, durations and purposes, verified in Arc on September 17. Arian gets his catalog by default; other senders only when the instruction explicitly names Arian. Explicit unsupported numeric booking durations stop before the model and preserve the existing reply with a visible correction prompt. There is no 15-minute event; available durations are 20, 30 and 60 minutes. The model must not invent or rewrite links/durations or claim slot availability. Draft success still replaces the reply editor directly; Queue reply is still the only send-enqueue path. No Cal.com writes, new migrations, public-comment/summary changes or campaign changes are included. The user approved publication on September 17; live Slack UI verification remains separate from Git publication.

**Optional guidance QA:** `artifacts/qa/campaigns/20260917T191459140640Z/` passed 188 isolated checks; the two unchanged deployment-SDK checks skipped in that environment. Coverage includes instruction-bound dedupe/recovery, changed/cleared guidance, editor preservation, manual-submit isolation, missing-duration feedback, calendar ownership, and existing Slack/comment behavior. Five synthetic draft-only calls through the configured model also selected the exact quick-chat, intro, next-steps and deep-dive URLs and omitted a link when instructed. These calls performed no DB, Slack or outbound-send operations. This is local implementation/model QA, not a deployed Slack UI verification.

**Private draft QA (2026-09-17):** The focused disposable-PostgreSQL run in `artifacts/qa/campaigns/20260917T150636658780Z/` passed 160 checks, including concurrent claims, migration/model consistency, expired-lease fencing, saved-result reuse, slow model responses, deployment dependency parity, and existing Slack/comment/manual-submit regressions. The two deployment-SDK tests skipped there passed separately on September 17 with public Vercel SDK 0.11.2 and runtime 0.23.0 in isolated temporary directories. These offline tests used no shared DB, model endpoint or Slack workspace. Live rollout evidence is recorded separately below.

**Private draft rollout verified (2026-09-17):** The user approved publication and rollout. Only migration `0034_slackreplydraftjob` was pending and was applied; all shared-DB migrations then matched the local graph. Deployment `dpl_BkzoZWo45aXt1oy2xPyoB68HLAbx` was promoted to `openoutreach.vercel.app` after isolated-package import and unsigned-request 401 checks. It supersedes the briefly deployed suggestion/Use draft UI at the user's request. Live private reply QA on Lead 11376/Chuka confirmed successful generation replaces the input directly, with no suggestion box or Use draft button. The test modal was cancelled without Queue reply; DB readback confirmed the saved job is ready and zero manual-reply Tasks exist for that Lead. This verifies the reply-modal lane, not every future provider outage or the separate Lead-context draft entry point. No campaign, sender controls, delivery schedule or outbound Task was changed by QA. Earlier unpromoted candidates caught a requirements-include error and missing pure LinkedIn status helpers in the temporary package; neither replaced the live Slack endpoint. Private deployment evidence lives under `artifacts/qa/slack-private-draft-2026-09-17/`. Existing remote supervisors may pick up the main commit normally; no explicit sender restart is part of this release.

**Private Slack draft background execution (2026-09-17):** `api/slack_reply_drafts.py` owns only private reply drafting/recovery. The existing signed `/api/slack_enrich` router registers a coroutine with the public Vercel SDK and immediately ACKs; the streaming runtime drains that registered work after the response. No unmanaged thread, new daemon or outbound Task is introduced. Function-scoped requirements live in `requirements/slack.txt`; `api/requirements.txt` is a flat, test-checked deployment mirror without altering sender environments. The Vercel builder rejected a parent-relative requirements include. The function duration is 120 seconds; draft jobs are bounded to 90 seconds, model generation to 55 seconds / two 25-second attempts, and individual Slack/DB operations are bounded. Migration 0034 creates `SlackReplyDraftJob` receipts scoped to exact Lead, sender, thread and modal view. A unique revision-derived key and row-locked expiring lease prevent duplicate generation; token-conditional completion rejects replaced workers. Only explicit Check/Retry may reclaim an expired/failed lease. Generated copy is committed before the Slack update and reused on recovery; existing scoped artifacts are updated only when not newer than the job. This is a best-effort runtime-held job with manual recovery, not a durable auto-retry queue. A platform termination before result persistence may require regeneration after lease expiry.

Private draft modal updates are hash-conditional. Loading and failures preserve existing editor IDs/values. Per the user's September 17 request, successful Draft reply completion replaces the input directly using a new block ID and the same body action ID; there is no separate suggestion box or Use draft button. The draft action authorizes replacement when ready, including overwriting typing done while generation is pending. Queue reply remains the sole explicit send-enqueue action. Stale/closed views are never reopened and updates never omit the expected hash. Model retries do not retry actual sends. General summaries, public comment drafting, campaign scheduling and all sender workers retain their original paths. `scripts/qa_campaigns.py --suite slack-drafts` runs this lane and existing Slack regressions against a disposable PostgreSQL cluster; SDK collector tests additionally require the deployment SDK/runtime installed. Offline checks alone do not prove live health.

Slack Lead context displays `Lead.role_tag` as Role, with `Not assigned` for blank tags. The modal omits the Campaign/deal context and Recent LinkedIn messages sections; their underlying data remains available to AI context generation, and the separate reply-modal transcript is unchanged. This is display-only and requires no migration.

**Slack reply-draft ICP references (2026-09-16):** `fetch_reply_draft_context` wraps the existing context loader only for the two private reply-draft actions. Its raw-SQL `fetch_reply_icp_messaging` matches the exact Lead/canonical sender, rejects conflicting thread owners, and uses exact thread enrollment evidence when available or a unique enrollment with confirmed LinkedIn sends otherwise. Ambiguous/missing/unsupported matches omit examples rather than infer a cohort. It reads only that enrollment's frozen message version, filters the exact audience and effective sender overrides, and passes the approved LinkedIn/email examples plus reviewed Role Tag into the reply prompt. The frozen audience replaces mutable Lead ICP only in this transient payload; other campaigns' Deal context is excluded. No Sheet/JSON import, Django dependency in the serverless path, delivery creation, lead mutation or new schema is added. Conversation content takes precedence, templates are explicitly references rather than sent-message history or regulatory proof, and generated text remains draft-only until manual submission. The general summary/context modal and public-comment lane retain their existing paths. Regression checks cover isolation, fallback, sender overrides, prompt contents and no enqueue; read-only checks matched eight real sender/cohort enrollments and rejected their opposite senders. No production deployment or live model call is implied.
...
Successful AI comment drafts increment a transient `comment_revision` in modal metadata and use a new comment input block ID. Slack preserves entered values when both block and action IDs match, so changing only `initial_value` leaves blank or old text visible. Loading/error updates retain the current input identity and sender selector; successful regeneration replaces only the editor. Parsing still finds the stable body action ID, and empty-comment validation targets the current input block. Drafting performs no enqueue or posting. Focused Slack regressions simulate Slack's preserved input state across blank/typed drafts, repeated generation, manual edits, model failure, sender selection, and submission.

The draft-replacement fix was deployed to Vercel production on September 14, 2026 as `dpl_8SS6tSFJa2pWCBnYz9QNEEuSe1Jm`, using the prior production source plus only the comment-module change. Package validation passed 78 Slack tests and the production handler passed an unsigned-request smoke check; this does not constitute a live Slack modal or LinkedIn posting test. The deployment receipt is under `artifacts/qa/slack-comment-draft-2026-09-14/`. On September 15 the user approved publishing both Slack edits to main; the combined tree passed 133 isolated Slack/feed regressions and 3 offline browser checks after incorporating main's feed-recovery updates. These changes require no migration or Gmail scheduling change; a Git push does not itself prove production rollout.
...
- **`actions/`** — `connect.py` (`send_connection_request`; invite-note UI also recognizes LinkedIn's current `We know each other` textarea placeholder), `status.py` (`get_connection_status`), `message.py` (`send_raw_message` plus exact-URN `send_direct_message_once` with optional attachment readiness polling and one-click outcome classification), `profile.py` (profile extraction), `search.py` (LinkedIn search), `conversations.py` (`get_conversation`).
...
- **`notifications/crm_sheets.py`** — shared safe Sheet primitives plus retired-surface adapters used only for exact first-cutover import, inventory, backup, and recovery. Legacy Opportunities, Pipeline, Recovery, and sender Followups are not canonical publication targets after v2 activation. New plan rows use native `appendCells` after all populated rows, preserving raw strings and bypassing logical-table detection at gaps. This prevents overwriting later stable IDs/operator cells when inactive projection rows have blank visible columns; staged verification and atomic activation remain mandatory.
...
**Browser recovery and diagnostics.** The collector allows three bounded browser attempts for Playwright timeouts and closed/crashed targets. Every attempt creates a fresh CDP transport and a new collector-owned page; retrying `goto` on a stalled target cannot recover an unresponsive renderer. Cleanup closes only that page, never the daemon's persistent context or listener tab. Saved post identities and counters survive recovery, while per-page traversal/idle detection starts over to let a replacement page scroll past already-saved content. Unexpected application/database failures are logged and re-raised without retries. A rotating `data/logs/linkedin-feed-collector.log` records job identity, navigation status, scan timing/counts, recoveries and final errors without cookies or post/message bodies.

The owned tab is created with `Target.createTarget(url=FEED_URL)` so its initial navigation starts at the browser level. The separate `new_page()` then `goto()` sequence reproduced an unresponsive blank target in the shared Windows Chromium session, even with fresh connections. The collector resolves the returned exact target ID to a Playwright page, waits for the feed document within the startup deadline, and closes that ID on every exit path. Concurrent tabs from other workers are never selected by URL/order or closed. It still uses the existing authenticated persistent browser, not a new login or profile.
