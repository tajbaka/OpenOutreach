...
- **LinkedInProfile** (`linkedin/models.py`) — 1:1 with User. Credentials, rate limits (`connect_daily_limit`, `connect_weekly_limit`, `follow_up_daily_limit`). Methods: `can_execute`/`rate_limit_reasons`/`reached_local_daily_connect_cap_only`/`record_action`/`mark_exhausted`. In-memory `_exhausted` dict records LinkedIn-reported daily exhaustion separately from DB-enforced daily/weekly/global caps.
...
- **Task** (`linkedin/models.py`) — `task_type` (`Task.TaskType` choices; `Task.save()` skips task-type choice validation so deploy-skew rows can still be marked failed), `status` (pending/running/completed/failed), `scheduled_at`, `payload` (JSONField), `error`, `started_at`, `completed_at`. Composite index on `(status, scheduled_at)`. Pending/running `sweep_connections` and `withdraw_invites` rows require `payload.operator` so each sender account runs only its own LinkedIn-account work.
...
- **`conf.py` stale invitation withdrawal** — `ENABLE_STALE_INVITE_WITHDRAWAL` (default `false`), `STALE_INVITE_AGE_DAYS` (30), and `STALE_INVITE_WITHDRAWAL_DAILY_LIMIT` (25 confirmed successes per sender-local day).
