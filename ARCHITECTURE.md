...
- **`conf.py` profile discovery** — `ENABLE_PROFILE_DISCOVERY` (default `false`), `DISCOVERY_DAILY_LIMIT` (25), `DISCOVERY_CONNECT_LIMIT_GRACE` (5), and hard card/page/profile-visit/consecutive-no-match/run-time caps. Daily boundaries use `ACTIVE_TIMEZONE`; there are no discovery start/end hours.
