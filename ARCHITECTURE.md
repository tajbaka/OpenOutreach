...
- **`notifications/attio.py`** — REST client for Attio. Sales-list entry `Stage` enum: Prospecting → Qualification → Meeting → Closing → Won (Lost terminates); `PROGRESSION_RANK` orders these and `should_patch_stage` blocks downgrades. `Closing` mirrors the Person `Prospecting to close` step and is human-driven (added 2026-04-28). Per-Person `outreach_status` ranks the full Attio enum (Invite Sent → Connected → Replied → Wants Meeting → Meeting Booked → Had Meeting → Prospecting to close → Won) and `should_patch_outreach_status` blocks demotion in either direction. Statuses 4-7 (Wants Meeting / Meeting Booked / Had Meeting / Prospecting to close) are emitted by humans or by the Phase D synthesis pass — never by `deal_to_outreach_status` (which only emits Invite Sent / Connected / Replied / Won / Lost). `create_person` and `create_company` POST plain creates — Attio's assert pattern (`PUT ?matching_attribute=...`) requires the field to be `is_unique=true` in the workspace, which `linkedin`/`name` aren't by default. Manual Attio inserts with the same LinkedIn URL or company name are NOT currently deduplicated; if dedupe matters, mark `linkedin` unique in Attio settings and swap `create_person` to `PUT ?matching_attribute=linkedin`. `add_person_email` appends to multiselect `email_addresses` (which IS unique, so it's idempotent). `create_person_note` POSTs notes for synthesis-pass auditability.
- **`notifications/synthesis.py`** — Synthesis pass invoked per-Deal from `sync_attio`. `extract_email_from_messages` (D1) regex-extracts email from inbound `crm.Message` rows. `detect_wants_meeting` (D2) calls a cheap LLM with structured Pydantic output over the unified thread. `synthesize_for_deal` orchestrates both with three gates: skip-when-already-detected (`Deal.wants_meeting_detected_at`), skip-when-no-new-messages (`Deal.last_synthesized_at` vs latest `Message.sent_at`), skip-when-status-already-past-Wants-Meeting. All failures logged; never break the surrounding Stage/Status sync.
