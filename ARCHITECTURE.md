...
- **`crm`** — durable contacts/messages/meetings plus the canonical sales domain (`SalesOwner`, `Account`, `Opportunity`, contacts, Actions, pipeline/Trello state, stage events, and Sheet baselines). Legacy `Deal` remains outreach automation state and defines `ClosingReason`.
...
- **Account / Opportunity** (`crm/models/sales.py`) — stable UUID-backed account sales motions, separate from Deal. Opportunity owns the explicit sender, legacy detailed stage, 15-step motion, last meaningful activity, manual pin, value/probability, and won/lost outcome. Its separate `pipeline_stage` is the high-level Trello stage; blank means broad-radar only. CRM v2 also persists whether it is an Active Account, the deterministic admission reason/reason set and evidence tier, evaluation time, and reversible inactive metadata. Weak automated rows are deactivated rather than deleted or closed.
...
- **OpportunityTrelloState / OpportunityPipelineEvent** (`crm/models/trello.py`) — one durable Opportunity-to-board/card/list mapping with the last published managed-card baseline, plus auditable high-level pipeline transitions. Card titles are never identity; the mapping and exact Opportunity UUID footer are both required.
...
- **`gmail/data_sync.py`** — Direct Gmail relationship/context helpers. Every exact email-bearing Lead is considered independently of legacy Deal state and `Lead.disqualified`; those remain send-suppression concerns. Exact identities are searched in batches of at most 40. A 500-hit truncated OR query is recursively split for fairness; a single noisy address is bounded at 2,000 hits and reported as capped. At most 80 unique threads are fetched per mailbox/run, with deferred/checkpoint telemetry for convergent follow-up runs. Drafts, scheduled mail, machine replies and list mail are excluded, full threads are date-clipped, group messages map per exact RFC participant, Send-As aliases resolve outbound ownership, and mailbox-local Gmail IDs are namespaced before `crm.Message` persistence. A separate metadata-only, newest-first 90-day scan inspects at most 500 message hits / 500 candidate threads and returns exact bidirectional external participants (including account provenance) that have no Lead; it never creates Leads or outbound work. Gmail-delivered Gemini/Meet notes persist into `crm.Meeting.gemini_notes_raw` with conservative title/date/lead matching. Synthetic note identity accepts exact normalized full-name/token evidence only; one-character initials and substring matches are prohibited, and an identity-invalid synthetic meeting is never silently reused.
- **Synthetic Gmail-note quarantine (`crm_v2_evidence.py`)** — every stored synthetic `gmail-note:`/`gmail_note_email` Meeting that fails the exact title/Lead validator is excluded before account evidence is grouped. The row remains intact for the private audit and one-ID repair workflow, but cannot contribute to account admission, Action generation, Stage projection/promotion, or Trello-card selection. Other independently valid evidence for the account remains usable.
- **`gmail/meeting_identity_repair.py`** — rollback-dry-run-first repair service for identity-invalid synthetic `gmail-note:` Meetings. It reassociates only one uniquely provable Lead, preserves the Meeting/note, and refuses manual/Sheet context, human revision/current action, or curated pipeline state. The operational runbook requires one explicit Meeting ID at a time; ambiguous records remain untouched.
...
- **Canonical CRM v2 Sheet projection (`crm_v2_publish.py`, `notifications/crm_v2_sheets.py`)** — the account-first serializer and safe adapters for exactly two concise surfaces: a broad, stable `Active Accounts` radar and one current `Actions` queue. Admission evidence is explicit (`Why active`, evidence tier, meaningful touch, `Attention`, and `Who owes`); source threads stay out of the working view. `Stage` is a system-owned `pipeline_stage` projection (`Radar only` when blank), not an imported Sheet edit. Primary/authoritative work may publish with Owner `Unassigned`; secondary LinkedIn-only work cannot create unowned queue clutter. Missing projection rows are cleared in place without deleting stable IDs, formulas, operator columns, or worksheets, and supported human cells retain the conservative three-way merge. Pipeline and Recovery Sheet tabs remain retired.
...
- **CRM v2 DB reconciliation (`crm_v2_reconcile.py`)** — the transactional write bridge from resolved account evidence to durable Account/primary Opportunity/contact state. Dry-run executes the apply path under rollback. Exact Opportunity/Lead links win; unanchored account domain/name ambiguity fails closed. Only admitted evidence creates rows, exact disqualified Leads remain linkable contacts, blank domains are filled only from one unique business domain, and stale bootstrap/system Opportunities are reversibly deactivated. Manual/Sheet Opportunities and pins are authoritative. The service never creates Actions or changes owner, legacy stage, sales-motion step, value, probability, or the separate pipeline stage; the latter has its own narrow policy.
- **Canonical CRM domain** — `crm.SalesOwner`, `Account`, `Opportunity`, `OpportunityContact`, `OpportunityAction`, `OpportunityStageEvent`, `OpportunitySheetState`, `OpportunityTrelloState`, and `OpportunityPipelineEvent` are UUID-backed durable sales state. Legacy `Deal` remains outreach automation state. `MeetingParticipant` and `MeetingNote` add multi-contact and Granola/Gemini context while legacy Meeting fields remain for additive rollout. `Message.operator` stores strict sender provenance when known.
- **CRM v2 admission/action policy** — `crm_v2_policy.py`, `crm_v2_evidence.py`, and `crm_v2_actions.py` admit accounts from explicit human/Sales Motion state, real meetings, human Gmail, or substantive bidirectional LinkedIn evidence in that order. One-sided outbound remains People-only. At most one current action is generated per opportunity; exact thread-level direction decides `Needs response` versus `Waiting`, primary/authoritative work may remain visible as `Unassigned`, secondary LinkedIn-only work may not, and exact target Don't send suppresses outreach without erasing sales relevance. Old recovery reviews require authoritative state plus one strong meeting/Gmail source or both strong source types; LinkedIn never widens recovery eligibility. Human current actions, owner, closure, and manual pins remain authoritative.
- **Curated pipeline policy (`crm_pipeline_policy.py`)** — separates broad radar admission from pipeline selection. It sets only a blank `pipeline_stage` to `Potential / Triage`, and only for authoritative account state or completed-meeting plus real-human-Gmail evidence. One meeting and every LinkedIn-only relationship remain radar-only. It never advances, demotes, or closes a pipeline stage.
- **Trello pipeline (`trello.py`, `trello_pipeline.py`)** — small HTTPS REST client plus locked, compare-before-write two-way projection for only nonblank pipeline stages. The exact ordered lists are Potential / Triage, Discovery, Demo / Evaluation, Pilot / Validation, Commercial / Procurement, Nurture / Later, Closed Won, and Closed Lost. First card placement comes from the DB stage; after mapping, list movement is the only supported human stage input. Stable `OpportunityTrelloState` plus the `OpenOutreach Opportunity ID: <UUID>` footer proves identity; names are never matched. Unknown/duplicate lists, unkeyed/duplicate/unknown cards, stale mappings, edited managed content, DB/Trello conflicts, and concurrent drift fail closed before writes. It never deletes/archives cards or copies conversations/transcripts. The Trello Free design deliberately avoids unavailable Custom Fields.
...
- **`management/commands/audit_gmail_note_meetings.py` / `repair_gmail_note_meetings.py`** — read-only audit and rollback-dry-run repair commands for historical synthetic note identity. Both keep stdout aggregate-only. Audit `--output` optionally creates a record-level JSON at mode 0600 and refuses overwrite. Repair requires explicit `--apply`; operators must scope and review one `--meeting-id` at a time rather than bulk-repairing ambiguous history.
...
- **`management/commands/sync_trello_pipeline.py`** — requires Trello key/token/board ID and defaults to strict no-write planning. `--apply` enables reviewed card/list and DB stage changes; the optional explicit `--bootstrap-lists` reports missing canonical lists in dry-run and creates only those lists at the bottom under apply. A second remote/DB comparison precedes card writes and final read-back must match.
...
- **Trello pipeline** — `TRELLO_API_KEY`, `TRELLO_API_TOKEN`, and `TRELLO_BOARD_ID` are required only by `sync_trello_pipeline`; `TRELLO_API_BASE` defaults to `https://api.trello.com/1` and `TRELLO_HTTP_TIMEOUT_SECONDS` to 30. Credentials are sent in the Authorization header and sanitized from errors. The board must be dedicated to the eight canonical open lists; default sync is read-only and writes require `--apply`.
