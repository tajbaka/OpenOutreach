...
- `LinkedInFeedPost` — canonical post record keyed by activity URN when available and a content hash fallback. Stores author metadata, an exact post permalink or activity-URN feed-update fallback, post text, raw extraction payload, and first/last seen timestamps. Cards without either kind of specific post URL are discarded before persistence; the upsert boundary enforces the same invariant. The raw payload retains candidate anchor links so permalink selector drift can be diagnosed later.
...
**Analyzer.** `manage.py analyze_linkedin_feed` has two modes and does not call an LLM. Export mode selects posts with `analyzed_at IS NULL` and a nonblank post URL, newest first, and writes every matching row to a JSON review queue for Codex (`--output`, `--since-days`, `--reanalyze`; `--limit` is only an optional manual/debug cap). Apply mode reads Codex-produced decision JSON (`--apply-json`) and saves `analyzed_at`, `intent` (`none/low/medium/high/urgent`), `audience` (`csp/advisor_partner/assessor/channel/other/not_relevant`), `topics`, `relevance_reason`, `suggested_action`, and `raw_analysis` onto `LinkedInFeedPost`. High/urgent posts in CSP/advisor/assessor/channel buckets post to `SLACK_HIGH_SIGNAL_URL` unless `--no-slack` is passed; successful alerts stamp `slack_notified_at`. Legacy URL-less rows are excluded both as alert candidates and as related posts in grouped alerts. The Codex review instructions explicitly include: people who want a GRC automation tool, FedRAMP tool, FedRAMP 20x tool, CMMC/FedRAMP/GRC help, or want to work as / find a GRC, FedRAMP, CMMC, 3PAO, assessor, advisor, channel, or partner resource. A Pete Strouse-style FedRAMP advisory/partner opportunity is high-intent.
