...
- **Account / Opportunity** (`crm/models/sales.py`) — stable UUID-backed account sales motions, separate from Deal. Opportunity owns the explicit sender, canonical stage and stage-entered time, sales-motion step, last meaningful activity, manual pin, value/probability, and won/lost outcome. Its stage is the only source for Pipeline placement.
...
- **`notifications/sheets.py`** — durable People publisher. Adds `Lead ID` only at the trailing edge, resolves stable ID before exact legacy LinkedIn URL, detects duplicate/ambiguous identities, indexes same-run appends immediately, and writes only managed changed cells. Rows are never cleared, reordered, or pruned; unknown columns and formula cells are not round-tripped through displayed values. `sync_sheets` is publication-only, shares `refresh_crm`'s cross-process advisory lock, and performs no LLM synthesis or sales eligibility decisions.
- **`notifications/crm_sheets.py`** — safe canonical/derived Sheet adapters. Opportunities use a non-pruning, DB-baseline-backed three-way merge of human cells; simultaneous DB/Sheet changes become explicit conflicts. Pipeline is stage-as-columns but consumes an explicit DB stage. Recovery and sender Followups replace managed cells in place by stable Opportunity/Action UUID without deleting worksheets; handled history and unsent human cells are retained. The module also provides read-only inventory and timestamped formula/value/schema backups.
...
- **Lifecycle policy** — `linkedin/crm_action_policy.py` is pure and date-bucketed in `ACTIVE_TIMEZONE`: 0–21 days may be Daily only when work is genuinely required, 22–60 days is Recovery, and over 60 days is archive/nurture. Due/overdue Actions, fresh inbound, a real upcoming meeting, or manual pin can override age. Future Waiting suppresses Daily until due. Terminal, Don't send, disqualified, failed-non-actionable, and polite-decline cases are excluded/reviewed before routing. Explicit Opportunity owner is authoritative; unowned current work is Recovery. Macro stages map sales-motion steps as Prospecting 1, Discovery 2, Demo Planning 3–4, Evaluation 5–6, Sandbox/Pilot 7–10, Commercial 11, Procurement/Legal 12–14, Expansion 15, plus human Closed Won/Lost.
- **Legacy Followup generator** — the former lead/name-based sheet rebuild is retained only behind explicit `generate_followups --legacy` for rollback. Canonical scheduling and publication consume persisted Actions through `refresh_crm`.
...
- **`management/commands/refresh_crm.py`** — the locked inspect/import/recalculate/publish orchestrator. Default is a live read-only Sheet plan plus rollback-only DB simulation; `--apply` backs up the workbook before structural writes, imports human changes, calls the shared date-bucket policy, publishes all surfaces, verifies People invariants, and records exact counts. The one-time Followups migration imports only positive stable matches and leaves unresolved material rows untouched and reported in dated `Legacy` tabs. Every affected canonical replacement is built and validated first, then all old/new sender-title pairs activate in one cohort-wide atomic Sheets batch; a preparation failure leaves the old canonical titles intact, and no legacy tab is deleted. A narrowly scoped `recover_failed_crm_sheet_titles()` helper handles the inverse failure mode (Sheets published, outer DB transaction rolled back) by revalidating exact source titles and IDs, renaming all generated surfaces to unique inspectable failed-output titles, and restoring all dated sender archives in one title-only atomic batch without deleting or editing cells.
- **`management/commands/generate_followups.py`** — canonical Codex draft queue by default. Export contains only persisted, explicitly owned Daily Actions plus bounded conversation/context after eligibility. Apply validates Action/Opportunity/Lead IDs and the full semantic fingerprint atomically, fills only a blank draft/channel, and republishes through `refresh_crm`; it never sends. The former lead/name tab rebuild requires explicit `--legacy`.
...
- **CRM Sheets** — `GOOGLE_SHEETS_ID` and `GOOGLE_SHEETS_CREDENTIALS_PATH` are required by `sync_sheets` and `refresh_crm`; those commands fail closed rather than silently doing nothing. `GOOGLE_SHEETS_TAB_NAME` defaults to `People`. Live `refresh_crm --apply` also requires `SALES_MOTION_VERSIONS_GOOGLE_SHEETS_ID` as an identity guard for the separate read-only Sales Motion workbook, which must never be used as the CRM write target.
