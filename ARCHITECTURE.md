...
submits INSERT a daemon-dispatched `manual_reply` `Task`. The reply modal also
fetches the recent LinkedIn `crm.Message` thread via raw SQL and renders a
compact transcript above the reply textbox; if that best-effort fetch fails, the
modal still opens. The function uses raw
