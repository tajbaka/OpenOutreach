...
`manage.py withdraw_invitations --account primary|backfill [--since YYYY-MM-DD] --before YYYY-MM-DD [--limit N] [--apply]` is intentionally outside the Task queue and daemon. `--since` is an optional inclusive midnight boundary and `--before` is an exclusive boundary in `ACTIVE_TIMEZONE`; omitting `--limit` attempts every live matching invitation in the eligible date window, while providing it caps confirmed withdrawals, not CRM candidates processed. Default mode reads the DB only and prints the account/operator, scan and evidence counts, exclusions, selected date range, full newest-before-cutoff scan pool, and withdrawal target. The order is deliberate: LinkedIn's Sent Invitations UI is newest-first infinite scroll, so apply targets the newest eligible stale invites before the cutoff instead of forcing the browser through every newer invite to reach the oldest rows. `--apply` acquires a same-sender PostgreSQL advisory lock on a dedicated connection, rejects a fresh `DaemonHeartbeat` or persistent Chromium `Singleton*` marker, then opens `StandaloneLinkedInSession` and verifies Voyager's self-profile resolves to the requested operator.

Selection is account-wide and requires positive OpenOutreach attribution. Current rows need `invitation_sent_at` plus same-operator `invitation_sender`. Legacy rows need a nonblank `sent_note` and exactly one CONNECT `ActionLog` for the selected `LinkedInProfile` and same Campaign written from 0 through 10 seconds after the Deal's PENDING `update_date`; a log that can match two Deals is ambiguous and excludes both. PENDING alone, partial ledgers, other senders, missing public IDs, and multiple proven Deals for one LinkedIn profile are excluded. Before any click, `actions/invitations.py` opens `/mynetwork/invitation-manager/sent/` and gradually scrolls the actual `<main>` container with randomized ordinary mouse-wheel increments and pauses. It scans the live sent-invitation list until it finds all reviewed URL/name targets, reaches list end, hits the time cap, or reaches an approximate timeline depth based on visible `Sent N days/weeks/months ago` labels for the eligible date window. There is no hard scroll-round cap; rounds remain telemetry. The visible labels are only scroll anchors and can be off by a couple of days; withdrawal still matches only a card containing the exact canonical `/in/<public-id>/` profile link, the scoped `Withdraw invitation sent to <name>` control, and a displayed name consistent with the stored Lead. The click is scoped to that card; the confirmation must expose one unambiguous Withdraw control, and the exact card must disappear before success is persisted. Candidates absent from the scanned sent list or changed between scan and click keep their CRM ledger unchanged and remain eligible for the normal acceptance sweep. Once `--limit` confirmed withdrawals are persisted, apply stops clicking even if more live matches were found. A success transaction optionally backfills legacy sender/sent time, stamps `invitation_withdrawn_at`, marks the Deal FAILED/Failed without globally disqualifying the Lead, and writes exactly one `ActionLog.WITHDRAW_INVITE`; the post-click write retries once on a dead DB connection. The same operator's automatic connect lane continues to reject any Lead with confirmed withdrawal history.
...
- **`actions/invitations.py`** — Human-paced Sent Invitations manager scanner, exact profile-card/name matching, scoped Withdraw dialog confirmation, and post-click card-disappearance verification.
- **`invitation_withdrawal.py`** — Account-wide positive-evidence planning, optional bounded date window, unique legacy connect-log matching, daemon/browser conflict checks, dedicated Postgres sender lock, Sent-card batch execution, and atomic/idempotent audit persistence.
...
- `LinkedInFeedPost` — canonical post record keyed by activity URN when available and a content hash fallback. Stores author metadata, exact post permalink when LinkedIn exposes one, an activity-URN feed-update fallback when available, post text, raw extraction payload, and first/last seen timestamps. The raw payload retains candidate anchor links so permalink selector drift can be diagnosed later.
...
**Analyzer.** `manage.py analyze_linkedin_feed` has two modes and does not call an LLM. Export mode selects posts with `analyzed_at IS NULL`, newest first, and writes every matching row to a JSON review queue for Codex (`--output`, `--since-days`, `--reanalyze`; `--limit` is only an optional manual/debug cap). Apply mode reads Codex-produced decision JSON (`--apply-json`) and saves `analyzed_at`, `intent` (`none/low/medium/high/urgent`), `audience` (`csp/advisor_partner/assessor/channel/other/not_relevant`), `topics`, `relevance_reason`, `suggested_action`, and `raw_analysis` onto `LinkedInFeedPost`. High/urgent posts in CSP/advisor/assessor/channel buckets post to `SLACK_HIGH_SIGNAL_URL` unless `--no-slack` is passed; successful alerts stamp `slack_notified_at`. The Codex review instructions explicitly include: people who want a GRC automation tool, FedRAMP tool, FedRAMP 20x tool, CMMC/FedRAMP/GRC help, or want to work as / find a GRC, FedRAMP, CMMC, 3PAO, assessor, advisor, channel, or partner resource. A Pete Strouse-style FedRAMP advisory/partner opportunity is high-intent.
